...
fn rot(ra: f32, dec: f32, roll: f32) -> Matrix3<f32> {
    let cam_z =
        Vector3::from_array([dec.cos() * ra.cos(), dec.cos() * ra.sin(), dec.sin()]).normalize();
    let north = Vector3::from_array([0.0, 0.0, 1.0]);
    let raw_x = north.cross(&cam_z);
    let x0 = if raw_x.norm() > 1e-6 {
        raw_x.normalize()
    } else {
    ...
    let y0 = cam_z.cross(&x0);
    let cam_x = x0 * roll.cos() + y0 * roll.sin();
    let cam_y = -x0 * roll.sin() + y0 * roll.cos();
    Matrix3::new([
